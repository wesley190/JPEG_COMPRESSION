...
void convolution_mask(YCBCR **input, YCBCR **output, int height, int width){
	int i,j,u,v;
    int test1,test2;
	double temp,out;
	...
	for(i=0;i<height;i++){
        for(j=0;j<width;j++){
        	out = 0.0;
        	for(u=0;u<3;u++){
        		test1 = i-1+u;//convolution 位置
        		for(v=0;v<3;v++){
        			test2 = j-1+v;
        			if(test1<0 || test1>=height || test2<0 || test2>=width){
        				temp = 0.0;
					}
					else{
						temp = input[test1][test2].Y;
					}        			
					out += temp * filter[u][v]/16;
				}
        	}
        	output[i][j].Y = out;
...
int main(int argc, char *argv[]){
    ...
    convolution_mask(R2Y, convol, height, width);
